// Workaround: each ROR KBN node loads new index settings only when its settings cache expires,
// up to ~10 s after this POST returns. A poll cannot detect this, because every check reads the
// settings in ES. Remove this wait when ROR KBN applies new settings before the POST returns.
